...
from fairseq.models.lra.prefixformer_lra_encoder import PrefixformerLRAEncoder
...
class LRAModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--k-times', type=int, metavar='N',
                            help='expansion multiples of K_{comp}, V_{comp} matrices in Prefixformer')
        ...
        parser.add_argument('--layer-type', choices=['transformer', 'luna', 'lstm', 'flash', 'mega', 'prefixformer'])
    ...
    def forward(self, sample):
        ...
        if not self.use_p:
            if self.layer_type in ['transformer', 'lstm', 'flash', 'mega', 'prefixformer', 'mamba','mamba2']:
    ...
    def build_model(cls, args, task):
        ...
        args.max_positions = args.max_len  # 1 2 3 4 5k length
...
class LRAEncoder(FairseqEncoder):
    ...
    def __init__(self, args, task):
        ...
        elif args.layer_type == 'prefixformer':
            self.encoder = PrefixformerLRAEncoder(
                tie_layer_weights=getattr(args, 'tie_layer_weights', False),
                padding_idx=padding_idx,
                vocab_size=vocab_size,
                num_encoder_layers=args.encoder_layers,
...
@register_model_architecture('lra', 'prefixformer_lra_listop')
def prefixformer_lra_listop(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'prefixformer')
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.activation_fn = getattr(args, 'activation_fn', 'silu')
...
@register_model_architecture('lra', 'prefixformer_lra_imdb')
def prefixformer_lra_imdb(args):
    args.layer_type = getattr(args, 'layer_type', 'prefixformer')
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.max_positions = getattr(args, 'max_positions', 4002)
    args.encoder_normalize_before = getattr(args, 'encoder_normalize_before', True)
...
@register_model_architecture('lra', 'prefixformer_lra_aan')
def prefixformer_lra_aan(args):
    args.layer_type = getattr(args, 'layer_type', 'prefixformer')
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.max_positions = getattr(args, 'max_positions', 4002)
...
@register_model_architecture('lra', 'prefixformer_lra_cifar10')
def prefixformer_lra_cifar10(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'prefixformer')
...
@register_model_architecture('lra', 'prefixformer_lra_pf32')
def prefixformer_lra_pf32(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.encoder_learned_pos = getattr(args, 'encoder_learned_pos', False)
    args.layer_type = getattr(args, 'layer_type', 'prefixformer')
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
...
@register_model_architecture('lra', 'prefixformer_lra_pf128')
def prefixformer_lra_pf128(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'prefixformer')
    args.encoder_layers = getattr(args, 'encoder_layers', 4)
    args.activation_fn = getattr(args, 'activation_fn', 'silu')
