...
class LRAImageTask(FairseqTask):
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        param_dict = {
            ...
            "reverse_order": getattr(self.args, "reverse_order", False),
            "seed": self.args.seed,
        }
        ...
        cache_path = os.path.join('/workspace/lra_saved', f"{task_name}_{split}_cache_{param_hash}.pt")

        # 如果缓存存在，直接加载
        if os.path.exists(cache_path):
            logger.info(f"Loading dataset from cache: {cache_path}")
            self.datasets[split] = torch.load(cache_path, weights_only=False) # DW
            # self.datasets[split] = torch.load(cache_path)  # SDU
            return self.datasets[split]

        # 如果缓存不存在，打印提示信息（单行）
        logger.info(f"[CACHE INIT] First time caching for split={split}, params={param_dict}")
        ...
        if os.path.exists(label_path):
            label_dataset = RawLabelDataset(
                [int(line.strip()) for i, line in enumerate(open(label_path).readlines())]
            )
            dataset.update(target=label_dataset)
